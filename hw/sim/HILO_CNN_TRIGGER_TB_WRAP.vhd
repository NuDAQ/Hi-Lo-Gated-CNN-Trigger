...
--   adc_flat[1535 - (ch*32 + s)*12  -: 12]  ↔  adc_data4_type(ch)(s)
--   Total width: 4 channels × 32 samples × 12 bits = 1536 bits
-- =============================================================================
...
entity HILO_CNN_TRIGGER_TB_WRAP is
    port (
        ...
        -- Flat replacement for adc_data4_type (4 ch × 32 samples × 12 bits)
        ADC_DATA4_FLAT : in  std_logic_vector(1535 downto 0);
...
begin
    ...
    gen_ch : for ch in 0 to 3 generate
        gen_s : for s in 0 to 31 generate
            adc_internal(ch)(s) <=
                ADC_DATA4_FLAT(1535 - (ch * 32 + s) * 12 downto
                               1524 - (ch * 32 + s) * 12);
        end generate gen_s;
    end generate gen_ch;
